...
<div class="admin_component">
	...
	<div class="fm admin_view_article">
		<form id="component_article_form" action="<?=$this->uri->full_url('/admin/loyalty_system/update_article');?>" method="post">
			<input type="hidden" name="component_id" value="<?=$component_id;?>">
			<input type="hidden" name="menu_id" value="<?=$menu_id;?>">
			<?php foreach ($languages as $key => $val): ?>
			<div id="article_tab_<?=$key;?>" class="article_tab"<?=(($key != LANG) ? ' style="display: none"' : ''); ?>>
				<div class="evry_title">
					<textarea class="loyalty" id="text_<?=$key;?>" name="text" style="width: 95%; height: 1000px; margin-left: 15px;">
						<?php echo $text['text'];?>
					</textarea>
				</div>
			<?php endforeach; ?>
...
<script type="text/javascript">
	...
	$('.loyalty').ckeditor({height: 200});

	function recreation_component_article()
	{
		component_loader_show($('.component_loader'), '');

		var uri = '<?=$this->uri->full_url('admin/loyalty_system/update_article');?>',
	...
	$(function () {
		...
		$('.component_edit_links .save, .for_sucsess .save_adm').on('click', function (e) {
			e.preventDefault();
			$('.loyalty').ckeditor({action: 'update'});
