...
?>
<div class="admin_component" id="admin_component_<?=$component_id;?>" data-component-id="<?=$component_id;?>" data-menu-id="<?=$menu_id;?>" data-module="services" data-css-class="services" data-visibility-url="<?=$this->uri->full_url('admin/components/toggle_visibility');?>" data-delete-url="<?=$this->uri->full_url('admin/services/delete_component');?>">
	<div class="component_loader"><span></span></div>
	<div class="fm adcom_panel">
		<div class="fm type_of_component">
			<div class="<?=(($hidden == 0) ? 'services' : 'hidden');?>"></div>
		</div>
		<div class="fm component_edit_links">
			<a class="fm add" href="#" id="add_services"><b></b>Додати новий сервіс</a>
	...
	<div class="fm admin_menu">
		...
		<ul class="admin_services">
			<?php if (isset($services) AND count($services) > 0): ?>
				<?php foreach ($services as $row): ?>
					<li data-id="<?=$row['services_id'];?>">
<!--                        --><?php //echo '<pre>';print_r($row);exit;?>
						<div class="holder">
							<div class="cell last_edit<?php if ($row['services_id'] == $last) echo ' active'; ?>"></div>
							<div class="cell w_30 number"></div>
							<div class="cell w_20 icon"><a href="#" class="hide-show<?php if ($row['hidden'] == 0) echo ' active'; ?>"></a></div>
							<div class="cell w_20 icon"><a href="<?=$this->uri->full_url('admin/services/edit?menu_id=' . $menu_id . '&services_id=' . $row['services_id']);?>" class="edit"></a></div>
							<div class="cell auto"><?=(($row['title'] != '') ? $row['title'] : 'Сервіс без назви');?></div>
...
//<![CDATA[
	$(document).ready(function () {
		...
		function row_decor() {
			$component.find('.admin_services').find('li')
				.removeClass('grey')
				.each(function () {
		...
		$component.on('click', '#add_services', function (e) {
			e.preventDefault();
			...
			$.post(
				...
				function (response) {
					if (response.success) {
						...
						var row = $('<li data-id="' + response.services_id + '"><div class="holder"><div class="cell last_edit active"></div><div class="cell w_30 number"></div><div class="cell w_20 icon"><a href="#" class="hide-show active"></a></div><div class="cell w_20 icon"><a href="<?=$this->uri->full_url('admin/services/edit?menu_id=' . $menu_id . '&services_id=');?>' + response.services_id + '" class="edit"></a></div><div class="cell auto">Сервіс без назви</div><div class="cell w_20 icon sorter"><a href="#" class="single_arrows"></a></div><div class="cell w_20 icon"><a href="#" class="delete"></a></div></div></li>');
						$component.find('.admin_services').prepend(row);
						row_decor();
		...
		if (jQuery.ui) {
			$component.sortable({
				...
				update: function (e, obj) {
					...
					$component.find('.admin_services').find('li').each(function () {
					...
					if (position.length > 0) {
						component_loader_show($('.component_loader'), '');
						$.post(
							'<?=$this->uri->full_url('admin/services/save_position');?>',
							{
								menu_id: <?=$menu_id?>,
								position: position
							},
		...
		$component
			.on('click', '.hide-show', function (e) {
				...
				$.post(
					'<?=$this->uri->full_url('admin/services/visible');?>',
					{
						menu_id: <?=$menu_id?>,
						services_id: $link.closest('li').data('id'),
						status: status
					},
					function (response) {
						if (response.success) {
			...
			.on('mouseover', '.admin_services .holder', function () {
				$(this).addClass('active');
			})
			.on('mouseout', '.admin_services .holder', function () {
