<?php
	...
	/**
	 * @property Admin_swiper_model $admin_swiper_model
	 */

	class Admin_swiper extends MX_Controller
	{
		...
		public function index()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id))
			{
				...
				$this->load->model('admin_swiper_model');

				$tpl_data = array(
					'menu_id' => $menu_id,
					'slides' => $this->admin_swiper_model->get_slides($menu_id),
					'last_slide' => $this->session->userdata('last_slide'),
					'thumb' => $this->thumb_sizes,
				);
				$this->template_lib->set_content($this->load->view('admin/swiper_tpl', $tpl_data, TRUE));
			}
			else
			{
		...
		public function add()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id))
			{
				...
				$response['slide_id'] = $this->admin_swiper_model->add(intval($this->input->post('add_top')), $menu_id);
		...
		public function edit()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id) AND $slide_id > 0)
			{
				$this->init_model->set_menu_id($menu_id, TRUE);
				...
				$tpl_data = array(
					...
					'slide' => $this->admin_swiper_model->get($slide_id),
					'thumb' => $this->thumb_sizes,
		...
		public function save()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id))
			{
				...
				$this->admin_swiper_model->update($set, $where);
		...
		public function upload()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id) AND $slide_id > 0)
			{
				...
				if ($this->upload->do_upload('slide_image'))
				{
					...
					$old_file_name = $this->admin_swiper_model->get_image($slide_id, LANG);
					...
					$this->admin_swiper_model->update_image($slide_id, $file_name);
		...
		public function crop()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('swiper_index', $menu_id))
			{
				if ($coords['x'] < 0) $coords['x'] = 0;
				if ($coords['y'] < 0) $coords['y'] = 0;
					if ($coords['x'] >= 0 AND $coords['y'] >= 0 AND $coords['w'] > 0 AND $coords['h'] > 0 AND $image_id > 0)
					{
						$image = $this->db->select('file_name_'.LANG.' as file_name, slide_id')->where('slide_id', $image_id)->get('swiper')->row_array();

						if (count($image) > 0)
						{
							$dir = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/upload/swiper/' . $menu_id . '/' . $image['slide_id'] . '/';
		...
		public function delete_photo()
		{
			...
			{
				$this->load->model('admin_swiper_model');
				$file_name = $this->admin_swiper_model->get_image($slide_id, LANG);

				if ($file_name !== NULL AND $file_name != '')
				{
					$dir = ROOT_PATH . 'upload/swiper/' . $menu_id . '/' . $slide_id . '/';
					...
					if ($file_name != '')
					{
						...
						$this->admin_swiper_model->update_image($slide_id, '', LANG);
					}
				}
		...
		public function hidden()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id) AND $slide_id > 0 AND is_numeric($hidden))
			{
				$this->load->model('admin_swiper_model');
		...
		public function slides_position()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id) AND is_array($slides))
			{
				$this->load->model('admin_swiper_model');
		...
		public function delete()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_swiper', $menu_id) AND $slide_id > 0)
			{
				...
				$this->load->model('admin_swiper_model');
				$this->admin_swiper_model->delete($slide_id);
		...
		public function delete_component()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_index', $menu_id) AND $menu_id > 0 AND $component_id > 0)
			{
				...
				$this->admin_swiper_model->delete_component($component_id);
