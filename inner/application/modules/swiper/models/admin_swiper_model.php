<?php
	...
	class Admin_swiper_model extends CI_Model
	{
		/**
		...
		public function get_slides($menu_id)
		{
			...
			return $this->db->get('swiper')->result_array();
		}
		...
		{
			$position = $this->db->select_max('position', 'max')->get('swiper')->row_array();
		...
		{
			$this->db->set('position', '`position` + 1', FALSE)->update('swiper');

			$position = $this->db->select_min('position', 'min')->get('swiper')->row_array();
		...
		public function get($slide_id)
		{
			...
			return $this->db->get_where('swiper', array('slide_id' => $slide_id))->row_array();
		}
		...
		{
			$this->db->update('swiper', $set, $where);
		}
		...
		{
			$result = $this->db->select('file_name_'.$language . ' as file_name')->get_where('swiper', array('slide_id' => $slide_id))->row_array();
		...
		{
			$this->db->set('file_name_'.LANG, $file_name)->where('slide_id', $slide_id)->update('swiper');
		}
		...
		{
			$this->db->delete('swiper', array('slide_id' => $slide_id));
		}
		...
		public function delete_component($component_id)
		{
			...
			$this->db->delete('swiper', array('menu_id' => 1));
