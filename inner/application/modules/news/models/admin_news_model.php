<?php
	...
	class Admin_news_model extends CI_Model
	{
		...
		public function get_description($news_id)
		{
			$this->db->select('title_'.LANG.' as title, text_'.LANG.' as text');
		...
		public function update_photo($image_id, array $set)
		{
			$this->db->update(
				'news_images',
				$set,
		 ...
		 * @param array $where
		 */
		public function update($set, $where,$option=0)
		{
			...
			if($option==0){
				...
				{
					$url = $set['url_' . $val];
					$count = $this->db->get_where('news', array('url_'.$val => $url))->result_array();
					if($count > 0){
						$url .= '-'.$where['news_id'];
						$this->db->update('news', array('url_'.$val => $url), $where);
					}
		...
		public function get_component_byMenu_id($menu_id, $module){
			return $this->db->get_where('components', array('menu_id' => $menu_id, 'module' => $module))->row_array();
		}
		/**
		...
		public function delete_image($image_id)
		{
			...
			if (count($im) > 0)
			{
				$dir = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/upload/news/' . get_dir_code($im['news_id']) . '/' . $im['news_id'] . '/';
				...
				if (file_exists($dir . 'b_' . $file_name)) unlink($dir . 'b_' . $file_name);
				if (file_exists($dir . 'm_' . $file_name)) unlink($dir . 'm_' . $file_name);
		...
		public function delete($news_id)
		{
			...
			if (count($result) > 0)
			{
				$dir = ROOT_PATH . 'upload/news/' . get_dir_code($news_id) . '/' . $news_id . '/';
		...
		public function delete_component($component_id)
		{
			...
			foreach ($news as $v)
			{
				...
				$dir = ROOT_PATH . 'upload/news/' . get_dir_code($v['news_id']) . '/' . $v['news_id'];
				delete_files($dir, TRUE, FALSE, 1);
			}
