<?php
	...
	class Admin_news extends MX_Controller
	{
		...
		public function get_data()
		{
			...
			$this->load->model('admin_news_model');
			$result = $this->admin_news_model->get_description($element_id);
			if(!empty($result)){
				$response['success'] = TRUE;
		...
		public function component_visible(){
			$response = array('success' => FALSE);
			$menu_id = intval($this->input->post('menu_id'));
			$component_id = intval($this->input->post('component_id'));
			$status = intval($this->input->post('status'));
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id) AND $component_id > 0 AND in_array($status, array(0, 1)))
			{
				$this->load->model('admin_news_model');
				$set = array('hidden' => $status);
				$where = array('component_id' => $component_id);
                $this->db->update('components', $set, $where);
				$response['success'] = TRUE;
			}
			return json_encode($response);
		}
		...
		public function edit()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id) AND $news_id > 0)
			{
				...
				$this->session->set_userdata('last_news', $news_id);
				$this->load->model('admin_news_model');
				$component = $this->admin_news_model->get_component_byMenu_id($menu_id, "news");
				$tpl_data = array(
				...
				$tpl_data['included'] = unserialize($tpl_data['news']['included']);
		...
		public function save()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id))
			{
				...
				$on_main = $this->input->post('on_main');
				$date = time();
				$set = array('date' => $date);
				...
				if($on_main == 'on') $on_main=1; else $on_main=0;
				$set['on_main'] = $on_main;
		...
		public function position_photos()
		{
			...
			if (
				...
				foreach ($photos as $position => $photo_id) {
					$this->admin_news_model->update_photo(
						(int)$photo_id,
						array(
		...
		public function upload_image()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id) AND $component_id > 0 AND $news_id > 0)
			{
				...
				if ($this->upload->do_upload('news_image'))
				{
					...
					$this->image_lib->resize_crop($dir . $file_name, $dir ."b_". $file_name, $this->big[0], $this->big[1], false);
					$this->image_lib->resize_crop($dir .'s_'. $file_name, $dir .'t_'. $file_name, $this->image_thumb[0], $this->image_thumb[1], false);
					// $this->image_lib->resize_crop($dir .'s_'. $file_name, $dir .'m_'. $file_name, 230, 215);
					$this->load->model('admin_news_model');
					$set = array(
						'news_id' => $news_id,
						'component_id' => $component_id,
		...
		public function crop_image()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id))
			{
				...
				if ($coords['x'] >= 0 AND $coords['y'] >= 0 AND $coords['w'] > 0 AND $coords['h'] > 0 AND $image_id > 0)
				{
					...
					if (count($image) > 0)
					{
						$dir = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/upload/news/' . get_dir_code($image['news_id']) . '/' . $image['news_id'] . '/';
						if (file_exists($dir . 's_' . $image['file_name']))
						{
							...
							// кшо превюв і вилике зображення різних пропорцій
							if($x2 > $sizes[1]) {
								$midle = ($x2-$y2)/2;
								$tx = $midle;
								$tx2 = $y2+$midle;
							...
							$this->load->library('Image_lib');
							$this->image_lib->crop($dir . 's_' . $image['file_name'], $dir .'b_'. $image['file_name'], $x2 - $x, $y2 - $y, $x, $y);
							$this->image_lib->resize_crop($dir . 'b_' . $image['file_name'], $dir . 'b_' . $image['file_name'], $this->big[0], $this->big[1]);
							
							$this->image_lib->crop($dir . 's_' . $image['file_name'], $dir .'t_'. $image['file_name'], $tx2 - $tx, $ty2 - $ty, $tx, $ty);
							$this->image_lib->resize($dir . 't_' . $image['file_name'], $dir . 't_' . $image['file_name'], $this->image_thumb[0], $this->image_thumb[1]);
							$response['src'] = '/upload/news/' . get_dir_code($image['news_id']) . '/' . $image['news_id'] . '/t_' . $image['file_name'] . '?t' . time();
							$response['success'] = TRUE;
						}
					}
				}
		...
		public function watermark_image()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('news_index', $menu_id) AND $position > 0 AND $image_id > 0 AND $this->config->item('watermark') != '')
			{
				...
				if (count($result) > 0)
				{
					$dir_code = get_dir_code($result['news_id']);
					$dir = ROOT_PATH . 'upload/news/' . $dir_code . '/' . $result['news_id'] . '/';
					if (file_exists($dir . 's_' . $result['file_name']))
					{
						$this->load->library('Image_lib');
						copy($dir . 's_' . $result['image'], $dir . $result['file_name']);
						$this->image_lib->watermark (
							$dir .'s_'. $result['file_name'],
							$dir . $result['file_name'],
							ROOT_PATH . 'upload/watermarks/' . $this->config->item('watermark'),
							$position,
