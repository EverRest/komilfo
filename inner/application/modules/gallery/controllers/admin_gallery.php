<?php
	...
	/**
	 * @property Admin_gallery_model $admin_gallery_model
	 */

	class Admin_gallery extends MX_Controller
	{
		protected $thumb_sizes=array(245, 245);
		...
		public function index()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id))
			{
				...
				$this->template_lib->set_admin_menu_active('gallery', 'sub_level');

				$this->load->model('admin_gallery_model');

				$tpl_data = array(
					'menu_id' => $menu_id,
					'slides' => $this->admin_gallery_model->get_slides($menu_id),
					'last_slide' => $this->session->userdata('last_slide'),
					'thumb' => $this->thumb_sizes,
				);
				$this->template_lib->set_content($this->load->view('admin/gallery_tpl', $tpl_data, TRUE));
			}
			else
			{
		...
		public function add()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id))
			{
				...
				$response['slide_id'] = $this->admin_gallery_model->add(intval($this->input->post('add_top')), $menu_id);
		...
		public function edit()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id) AND $slide_id > 0)
			{
				$this->init_model->set_menu_id($menu_id, TRUE);
				...
				$tpl_data = array(
					...
					'slide' => $this->admin_gallery_model->get($slide_id),
					'thumb' => $this->thumb_sizes,
    ...
    //             $where = array('slide_id' => intval($this->input->post('slide_id')));
				// $this->admin_gallery_model->update($set, $where);
		...
		public function upload()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id) AND $slide_id > 0)
			{
				...
				if ($this->upload->do_upload('slide_image'))
				{
					...
					$old_file_name = $this->admin_gallery_model->get_image($slide_id, LANG);
					...
					$this->admin_gallery_model->update_image($slide_id, $file_name);
		...
		public function crop()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('gallery_index', $menu_id))
			{
				if ($coords['x'] < 0) $coords['x'] = 0;
				if ($coords['y'] < 0) $coords['y'] = 0;
					if ($coords['x'] >= 0 AND $coords['y'] >= 0 AND $coords['w'] > 0 AND $coords['h'] > 0 AND $image_id > 0)
					{
						$image = $this->db->select('file_name_'.LANG.' as file_name, slide_id')->where('slide_id', $image_id)->get('gallery')->row_array();

						if (count($image) > 0)
						{
							$dir = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/upload/gallery/' . $menu_id . '/' . $image['slide_id'] . '/';
							...
							if (file_exists($dir . 's_' . $image['file_name']))
							{
								...
								$response['success'] = TRUE;
								$response['image'] = '/upload/gallery/' . $menu_id . '/' . $image['slide_id'] .'/'. $image['file_name'] . '?t' . time();
							}
						}
					}
			}
		...
		public function delete_photo()
		{
			...
			{
				$this->load->model('admin_gallery_model');
				$file_name = $this->admin_gallery_model->get_image($slide_id, LANG);

				if ($file_name !== NULL AND $file_name != '')
				{
					$dir = ROOT_PATH . 'upload/gallery/' . $menu_id . '/' . $slide_id . '/';
					...
					if ($file_name != '')
					{
						...
						$this->admin_gallery_model->update_image($slide_id, '', LANG);
					}
				}
		...
		public function hidden()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id) AND $slide_id > 0 AND is_numeric($hidden))
			{
				$this->load->model('admin_gallery_model');
		...
		public function slides_position()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id) AND is_array($slides))
			{
				$this->load->model('admin_gallery_model');
		...
		public function delete()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_gallery', $menu_id) AND $slide_id > 0)
			{
				...
				$this->load->model('admin_gallery_model');
				$this->admin_gallery_model->delete($slide_id);
		...
		public function delete_component()
		{
			...
			if ($this->init_model->is_admin() AND $this->init_model->check_access('config_index', $menu_id) AND $menu_id > 0 AND $component_id > 0)
			{
				...
				$this->admin_gallery_model->delete_component($component_id);
