<?php
	...
	class Admin_gallery_model extends CI_Model
	{
		...
		{
			$position = $this->db->select_max('position', 'max')->get('gallery')->row_array();
		...
		{
			$this->db->set('position', '`position` + 1', FALSE)->update('gallery');

			$position = $this->db->select_min('position', 'min')->get('gallery')->row_array();
		...
		public function get($slide_id)
		{
			...
			return $this->db->get_where('gallery', array('slide_id' => $slide_id))->row_array();
		}
		...
		{
			$this->db->update('gallery', $set, $where);
		}
		...
		{
			$result = $this->db->select('file_name_'.$language . ' as file_name')->get_where('gallery', array('slide_id' => $slide_id))->row_array();
		...
		{
			$this->db->set('file_name_'.LANG, $file_name)->where('slide_id', $slide_id)->update('gallery');
		}
		...
		{
			$this->db->delete('gallery', array('slide_id' => $slide_id));
		}
		...
		public function delete_component($component_id)
		{
			...
			$this->db->delete('gallery', array('menu_id' => 1));
