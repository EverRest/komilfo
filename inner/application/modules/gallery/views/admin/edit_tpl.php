...
<div class="fm admin_component">
	...
	<form id="slide_form" action="<?=$this->uri->full_url('admin/gallery/save');?>" method="post">
		<input type="hidden" name="menu_id" value="<?=$menu_id;?>">
		<input type="hidden" name="slide_id" value="<?=$slide_id;?>">
		<?php foreach ($languages as $key => $val): ?>
		<?php
			$sizes = array(0, 0);
			if ($slide['file_name'] != '') $sizes = getimagesize(ROOT_PATH . 'upload/gallery/' . $menu_id . '/' . $slide_id . '/s_' . $slide['file_name']);
		?>
						...
						<li data-id="<?=$slide['slide_id'];?>" style="float: left; width:300px; height: 300px; margin-top: 2px; margin-left: 2px;">
								<div class="fm for_photo_cut">
									<div class="fm photo_cut" style="width:300px; height: 300px;">
										<?php $sizes = getimagesize(ROOT_PATH . 'upload/gallery/'. $menu_id . '/' . $slide['slide_id'] . '/' . $slide['file_name']); ?>
										<div style="width:300px; height: 300px;"><img src="/upload/gallery/<?=$menu_id;?>/<?=$slide['slide_id'];?>/<?=$slide['file_name'] . '?t=' . time() . rand(10000, 1000000);?>" alt="" style="width:300px; height: 300px;"></div>
										<div class="links">
											<a href="#" class="fm fpc_edit" data-image-id="<?=$slide['slide_id'];?>" data-src="/upload/gallery/<?=$menu_id;?>/<?=$slide['slide_id'];?>/s_<?=$slide['file_name'];?>" data-width="<?=$sizes[0];?>" data-height="<?=$sizes[1];?>"><b></b>Редагувати</a>
		...
		$('.component_lang').find('a').each(function () {
			 ...
			 */
					...
					$slider_image.fineUploader({
						request: {
							endpoint: '<?=$this->uri->full_url('admin/gallery/upload');?>',
										...
										<div class="fm for_photo_cut">\
											<div class="fm photo_cut" style="width: 300px; height: 300px">\
												<div style="width: 300px; height: 300px">\
													<img src="/upload/gallery/<?=$menu_id;?>/<?=$slide_id;?>/' + response.file_name + '" alt="" style="width: 300px; height: 300px;">\
												</div>\
												<div class="links">\
													<a href="#" class="fm fpc_edit" data-image-id="' + response.slide_id + '" data-src="/upload/gallery/<?=$menu_id;?>/<?=$slide_id;?>/s_' + response.file_name + '" data-width="' + response.width + '" data-height="' + response.height + '"><b></b>Редагувати</a>\
													<a href="#" class="fm fpc_delete"><b></b>Видалити</a>\
												</div>\
											</div>\
										</div>\
						...
						$('#images_list').on('click', '.fpc_delete', function (e) {
							...
							confirmation('Видалити зображення із слайдера?', function () {
								...
								$.post(
									'<?=$this->uri->full_url('admin/gallery/delete_photo');?>',
									{
								...
								$('#crop_source').find('img').Jcrop({
									...
									$('#crop_save').off('click').on('click', function (e) {
										...
										component_loader_show($('.component_loader'));
										$.post(
											'<?=$this->uri->full_url('admin/gallery/crop');?>',
											{
