						...
						<li data-id="<?=$slide['slide_id'];?>" style="float: left; width:300px; height: 300px; margin-top: 2px; margin-left: 2px;">
							<div class="fm for_photo_cut">
								<div class="fm photo_cut" style="width:300px; height: 300px;">
									...
									<div style="width:300px; height: 300px;"><img src="/upload/swiper/<?=$slide['file_name'] . '?t=' . time() . rand(10000, 1000000);?>" alt="" style="width:300px; height: 300px;"></div>
									<div class="links">
										<a href="#" class="fm fpc_edit" data-image-id="<?=$slide['slide_id'];?>" data-src="/upload/swiper/s_<?=$slide['file_name'];?>" data-width="<?=$sizes[0];?>" data-height="<?=$sizes[1];?>"><b></b>Редагувати</a>
...
<script type="text/javascript">
	$(document).ready(function () {
		 ...
		 */
		var $swiper_image = $('#slide_image');
		$swiper_image.fineUploader({
			request: {
				endpoint: '<?=$this->uri->full_url('admin/swiper/upload');?>',
				inputName: 'slide_image',
				params: {
					menu_id: <?=$slide['menu_id'];?>,
		...
		$('#images_list').on('click', '.fpc_delete', function (e) {
			...
			confirmation('Видалити зображення із слайдера?', function () {
				...
				$.post(
					'<?=$this->uri->full_url('admin/swiper/delete_photo');?>',
					{
