	...
	<div class="fm admin_menu">
		<ul>
			<li class="th">
				<div class="holder">
					...
					<div class="cell w_160" style="width: 170px;">Зображення</div>
		...
		<ul class="admin_catalog">
			...
			<?php foreach ($categories as $category): ?>
				<li id="catalog_<?=$category['id'];?>"  data-item-id="<?=$category['id'];?>">
					<div class="holder">
						<div class="cell w_20 icon"><a href="<?=$this->uri->full_url('admin/menu/edit?menu_index=1&menu_id=' . $category['id'] . '&item_id=' . $category['id']);?>" class="picture <?=($category['image'] != ''? 'active':'')?>"></a></div>
						<div style="width: 180px;"><?php if ($category['image'] !== ''): ?><img style="width: 180px;" src="<?=base_url('upload/menu/' . $category['id'] . '/' . $category['image']);?>" alt=""><?php endif; ?></div>
						<div class="cell auto"><span class="menu_item"><a href="<?=base_url($category['url_'.LANG])?>"><?=($category['name_'.LANG] === '' ? 'Новий запис' : $category['name_'.LANG]);?></a></span></div>
					</div>
				</li>
			<?php endforeach; ?>
		</ul>
