<?php
	...
	//$_site_menu = $admin['site_menu'] != '' ? explode(',', $admin['site_menu']) : array();
	//$_admin_menu = $admin['admin_menu'] != '' ? explode(',', $admin['admin_menu']) : array();
	$_site_menu = explode(',', $admin['site_menu']);
	$_admin_menu = explode(',', $admin['admin_menu']);
	$_rent = explode(',', $admin['rent']);
	$_long_rent = explode(',', $admin['long_rent']);
	$_sale = explode(',', $admin['sale']);
?>
<div class="admin_component" id="administrators_component">
	...
	<div class="fm adcom_panel">
		...
		<div class="fm component_edit_links">
			...
			<?php if ($this->session->userdata('admin_root') == 1): ?>
			...
			if($admin['root'] != 1): ?>
				...
				<div class="evry_title">
					...
					<div class="no_float admin_menu">
						<a href="#" id="check_user_menu" class="text_link">вибрати все</a>
						|
						<a href="#" id="uncheck_user_menu" class="text_link">зняти все</a>
						<br>
						<br>
				...
				<div class="evry_title">
					...
					<div class="no_float admin_menu">
						<a href="#" id="check_admin_menu" class="text_link">вибрати все</a>
						|
						<a href="#" id="uncheck_admin_menu" class="text_link">зняти все</a>
						<br>
						<br>
						<ul>
...
//<![CDATA[
	$(function () {
		...
		$('#check_user_menu').add('#check_admin_menu').add('#check_rent').add('#check_long_rent').add('#check_sale').on('click', function (e) {
			e.preventDefault();
			$(this).closest('.admin_menu').find('.check_label').addClass('active');
			$(this).closest('.admin_menu').find('[type="checkbox"]').prop('checked', true);
		});
		$('#uncheck_user_menu').add('#uncheck_admin_menu').add('#uncheck_rent').add('#uncheck_long_rent').add('#uncheck_sale').on('click', function (e) {
			e.preventDefault();
